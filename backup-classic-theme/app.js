...
// Theme toggle: Lux (default) <-> Classic. Reloads so the map basemap re-picks too.
document.getElementById('theme-btn').addEventListener('click', () => {
  const next = window.APP_THEME === 'lux' ? 'classic' : 'lux';
  try { localStorage.setItem('hhbl-theme', next); } catch (e) {}
  location.reload();
});
